...
function ensureUi() {
  ...
  // Remove only obsolete overlays once. Never remove the active overlay during render.
...
function finishArea() {
  ...
  // Preserve the mobile visual polygon immediately. The existing calculator only
  // needs a stable geometry payload to continue to the next step.
...
function stopDrag(id, source) {
  if (drag?.id === id && drag?.source === source) drag = null;
}
...
function installGlobalPanTracking() {
  ...
  // Document-level capture is necessary on iOS because GovMap may stop events
  // before they reach the map wrapper itself.
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    @media(max-width:960px){
      .solatrixGovMapCrosshair:before{background:radial-gradient(circle,#126eeb 0 4px,#fff 5px 9px,transparent 10px)!important}
    }
  `;
