...
function enhanceReportExperience() {
  ...
  offer.innerHTML = `
    ...
    <div class="reportWhatsappForm" data-report-form>
      ...
      <div class="reportWhatsappGrid">
        <label>
          <span>שם מלא *</span>
          <input name="name" autocomplete="name" value="${escapeAttr(originalName?.value || draft.name || '')}" />
        </label>
        <label>
          <span>מספר WhatsApp *</span>
          <input name="phone" autocomplete="tel" inputmode="tel" value="${escapeAttr(originalPhone?.value || draft.phone || '')}" />
        </label>
        <label class="wide">
          <span>אימייל (לא חובה)</span>
          <input name="email" autocomplete="email" type="email" value="${escapeAttr(originalEmail?.value || draft.email || '')}" />
        </label>
      </div>
      <label class="reportWhatsappConsent">
  ...
  offer.querySelector('[data-submit-report-request]')?.addEventListener('click', () => {
    submitWhatsappReport({ offer, reportCard, originalPdfButton, originalName, originalPhone, originalEmail });
  });
}

async function submitWhatsappReport({ offer, reportCard, originalPdfButton, originalName, originalPhone, originalEmail }) {
  ...
  button.textContent = 'מכינים את הדוח...';
  ...
  try {
    ...
    successNode.innerHTML = `<b>הבקשה התקבלה${reference ? ` — מספר פנייה ${reference}` : ''}.</b><br>הדוח נפתח לצפייה והבקשה לשליחה ב-WhatsApp נשמרה.`;
...
function syncOriginalField(field, value) {
  if (!field) return;
  field.value = value;
