...
function loadScript(src, ready, timeoutMs = 10000) {
  return new Promise((resolve, reject) => {
    ...
    const poll = () => {
      if (ready()) return resolve();
      if (Date.now() - started >= timeoutMs) return reject(new Error(`Timed out loading ${src}`));
...
async function resolveAddressPoint() {
  ...
  const selectedPoint = pointFromCandidate(selection?.result || selection);
  ...
  const response = await fetch(GOVMAP_AUTOCOMPLETE_URL, {
    ...
    body: JSON.stringify({
      ...
      maxResults: 15,
      isAccurate: true,
      ...(GOVMAP_TOKEN ? { apiKey: GOVMAP_TOKEN } : {})
    })
  });
...
function zoomToAddress() {
  if (!addressPoint || typeof window.govmap?.zoomToXY !== 'function') return false;
  try {
    window.govmap.setBackground?.(1);
    window.govmap.zoomToXY({ x: addressPoint.x, y: addressPoint.y, level: ADDRESS_ZOOM_LEVEL, marker: true });
    return true;
  } catch (error) {
    console.warn('GovMap zoom failed', error);
    return false;
  }
}
...
async function focusAddress() {
  try {
    ...
    addressPoint = await resolveAddressPoint();
    if (!addressPoint) {
      setHint('לא נמצאה כתובת מדויקת. חזרו ובחרו כתובת מרשימת GovMap.');
      return;
    }
    [0, 450, 1100, 2200, 3800].forEach((delay, index) => window.setTimeout(() => {
      if (zoomToAddress() && index === 0) {
        setHint(isMobile() ? 'הזיזו את המפה כך שפינת הגג תהיה מתחת לכוונת.' : 'הכתובת נמצאה. לחצו על פינות הגג כדי לסמן אותו.', true);
      }
    }, delay));
  } catch (error) {
    console.error('GovMap address focus failed', error);
    setHint('המפה נטענה, אך לא הצלחנו למקד את הכתובת. חזרו ובחרו כתובת מרשימת GovMap.');
  }
}
...
function startDraw() {
  ...
  else if (typeof request?.then === 'function') request.then(onResult).catch((error) => {
    drawing = false;
    console.error('GovMap drawing failed', error);
    setHint('הסימון נכשל. נסו שוב.');
  });
}
...
function dispatchCenterClick(double = false) {
  ...
  if (!map) return;
  const target = map.querySelector('canvas') || map.querySelector('iframe') || map;
...
function wireFooter() {
  ...
  if (!footer || footer.dataset.govmapWired === 'true') return;
  const actions = footer.querySelector('.compactActions') || footer.querySelector('.actions');
  if (!actions) return;
  footer.dataset.govmapWired = 'true';
  ...
  primary.addEventListener('click', (event) => {
    ...
    if (isMobile()) window.setTimeout(() => dispatchCenterClick(false), 90);
  });
...
async function initializeGovMap(panel) {
  await Promise.all([
  ...
  // iOS GovMap may render without exposing a canvas/image node. Never let the loading
  // overlay block the map while waiting for a DOM signature that may never appear.
  window.setTimeout(revealMap, 1600);
  window.setTimeout(revealMap, 3200);
  window.setTimeout(() => focusAddress(), 900);
}
...
async function install() {
  ...
  try {
    await initializeGovMap(panel);
  } catch (error) {
    installed = false;
    console.error('GovMap-only editor failed', error);
    panel.innerHTML = '<div class="solatrixGovMapHint">GovMap לא נטען. רעננו את הדף ונסו שוב.</div>';
  }
}
...
window.addEventListener('resize', () => { if (isRoofPage()) wireFooter(); });
window.addEventListener('popstate', () => window.setTimeout(tick, 100));
