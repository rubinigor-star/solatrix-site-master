...
function rtl(pdf,text,x,y,size,style='normal',color=C.navy,maxWidth){ pdf.setFont('Heebo',style);pdf.setFontSize(mm(size));pdf.setTextColor(...color);pdf.setR2L(true);const t=markNumbers(String(text??''));const lines=maxWidth?pdf.splitTextToSize(t,maxWidth):t;pdf.text(lines,x,y,{align:'right'});pdf.setR2L(false); }
...
function markNumbers(text){ return text.replace(/[0-9][0-9.,:%+\-]*/g,m=>`\u200E${m}\u200F`); }
