...
const C = { bg:[252,250,245], navy:[7,31,50], blue:[22,85,128], orange:[247,163,24], grey:[103,113,121], border:[226,220,208], white:[255,255,255], pale:[245,247,248], green:[34,166,91], grid:[224,228,231] };

export async function createRoofCheckPdf({ customer = {}, reportData = {} } = {}) {
  const roof = reportData.roofData || {};
  const rawModel = reportData.calculationModel || {};
  const values = buildValues(rawModel, reportData.calculation || {}, roof);
  ...
  page1(pdf, { customer, roof, values, generatedAt, logo, cover });
  pdf.addPage(); page2(pdf, { customer, values, logo, installers });
...
function buildValues(model, calculation, roof) {
  ...
  const economics = isCommercial ? calculateCommercialEconomics({ systemSizeKwp:size }) : calculateResidentialEconomics({ systemSizeKwp:size });
  const validation = validateEconomics(economics);
  return {
    ...economics,
    roofArea, usableArea, panels: panels || Math.max(0, Math.round(size / 0.63)),
    monthlyBill: num(model.monthlyBill, roof.monthlyBill),
    isCommercial, urbanEligible: model.urbanEligible === true || roof.urbanEligible === true,
...
function page1(pdf, c) {
  ...
  fact(pdf, 18, 238, 84, 20, 'כתובת הנכס', c.roof.address || '—', 'pin');
  fact(pdf, 108, 238, 84, 20, 'שם הלקוח', c.customer.name || '—', 'user');
  fact(pdf, 18, 263, 84, 17, 'תאריך הפקה', c.generatedAt, 'calendar', true);
  fact(pdf, 108, 263, 84, 17, 'שטח גג שסומן', `${number(c.values.roofArea)} מ״ר`, 'area', true);
}

function page2(pdf, c) {
  base(pdf, 2, c.logo);
  title(pdf, 'כך בנוי החישוב', 42);
  darkMain(pdf, 18, 50, 67, 69, 'ייצור שנתי', number(c.values.annualProduction), 'kWh', `${one(c.values.systemSizeKwp)} kWp × 1,650 kWh לשנה`);
  cover(pdf, c.installers, 90, 50, 102, 69, 6);
  section(pdf, 'פירוט הפרויקט', 132);
  const rows = [
    ['שטח גג מסומן', `${number(c.values.roofArea)} מ״ר`],
    ['שטח גג שמיש', `${number(c.values.usableArea)} מ״ר`],
    ...
    ['החזר השקעה', `${one(c.values.paybackWithVat)} שנים`]
  ];
  ...
  projectFact(pdf, 126, 196, 66, 23, 'WhatsApp', c.customer.phone || '—', 'whatsapp', true);
  projectFact(pdf, 126, 224, 66, 23, 'תקינות חישוב', c.values.validation.valid ? 'נבדק' : 'דורש בדיקה', 'check');
  ...
  note(pdf, 18, 273, 174, 10, 'הערה', 'המחיר הוא אומדן ראשוני להתקנה סטנדרטית וללא עבודות חריגות.');
}
...
function projection(v) {
  ...
  const months=weights.map((w,i)=>({label:labels[i],kwh:Math.round(v.annualProduction*w),money:Math.round(v.annualSavings*w)}));
...
function title(pdf,text,y){rtl(pdf,text,192,y,9.8,'bold',C.navy);}
function ribbon(pdf,x,y,w,h,text){round(pdf,x,y,w,h,C.orange,C.orange,4);rtl(pdf,text,x+w-7,y+8,4.2,'bold',C.navy);}
function metric(pdf,x,y,w,h,label,value,suffix,icon){card(pdf,x,y,w,h,5);iconDraw(pdf,icon,x+11,y+h/2);rtl(pdf,label,x+w-7,y+9,3.7,'bold',C.grey);ltr(pdf,value,x+w/2+4,y+21,7.2,'bold',C.navy,'center');if(suffix)ltr(pdf,suffix,x+w/2+4,y+27,3,'normal',C.grey,'center');}
function fact(pdf,x,y,w,h,label,value,icon,isLtr=false){card(pdf,x,y,w,h,4);iconDraw(pdf,icon,x+9,y+h/2);rtl(pdf,label,x+w-6,y+7,3.2,'normal',C.grey);isLtr?ltr(pdf,value,x+w-6,y+15,4.2,'bold',C.navy,'right'):rtl(pdf,value,x+w-6,y+15,4.2,'bold',C.navy,w-22);}
function band(pdf,x,y,w,h,titleText,text){round(pdf,x,y,w,h,C.navy,C.navy,6);rtl(pdf,titleText,x+w-8,y+11,6.1,'bold',C.white);rtl(pdf,text,x+w-8,y+21,3.8,'normal',[230,237,242],w-16);}
function darkMain(pdf,x,y,w,h,titleText,value,unit,text){round(pdf,x,y,w,h,C.navy,C.navy,6);iconDraw(pdf,'sun',x+w/2,y+13,C.orange);rtl(pdf,titleText,x+w-7,y+29,4.8,'bold',C.white);ltr(pdf,value,x+w/2,y+44,7.6,'bold',C.white,'center');ltr(pdf,unit,x+w/2,y+51,3,'normal',[225,233,239],'center');rtl(pdf,text,x+w-7,y+61,3.4,'normal',[225,233,239],w-14);}
function section(pdf,text,y){rtl(pdf,text,192,y,5.3,'bold',C.navy);}
function resultRow(pdf,x,y,w,h,label,value){card(pdf,x,y,w,h,3);rtl(pdf,label,x+w-5,y+7.3,3.3,'normal',C.grey);ltr(pdf,value,x+5,y+7.3,3.8,'bold',C.navy,'left');}
function projectFact(pdf,x,y,w,h,label,value,icon,isLtr=false){card(pdf,x,y,w,h,4);iconDraw(pdf,icon,x+9,y+h/2);rtl(pdf,label,x+w-6,y+8,3.2,'bold',C.grey);isLtr?ltr(pdf,value,x+w-6,y+17,4.2,'bold',C.navy,'right'):rtl(pdf,value,x+w-6,y+17,4.4,'bold',C.navy);}
function note(pdf,x,y,w,h,titleText,text){card(pdf,x,y,w,h,4);rtl(pdf,titleText,x+w-6,y+6.5,3.2,'bold',C.navy);rtl(pdf,text,x+w-6,y+12.5,3,'normal',C.grey,w-12);}
function analyticsMetric(pdf,x,y,w,h,label,value){card(pdf,x,y,w,h,4);rtl(pdf,label,x+w-5,y+8,3.1,'normal',C.grey);ltr(pdf,value,x+w/2,y+18,5.1,'bold',C.navy,'center');}
function chartCard(pdf,x,y,w,h){card(pdf,x,y,w,h,5);}
function monthlyChart(pdf,x,y,w,h,months){const max=Math.max(...months.map(m=>m.kwh),1),bw=8.4,gap=4.5,baseY=y+h;pdf.setDrawColor(...C.grid);for(let i=0;i<4;i++)pdf.line(x,y+i*h/3,x+w,y+i*h/3);months.forEach((m,i)=>{const bx=x+i*(bw+gap)+2,bh=(m.kwh/max)*(h-8);pdf.setFillColor(...C.orange);pdf.roundedRect(bx,baseY-bh,bw,bh,1.2,1.2,'F');ltr(pdf,m.label,bx+bw/2,baseY+5,2.4,'normal',C.grey,'center');ltr(pdf,number(m.kwh),bx+bw/2,baseY-bh-2,2.1,'bold',C.navy,'center');});}
...
function summaryBand(pdf,x,y,w,h,p,v){round(pdf,x,y,w,h,C.navy,C.navy,5);rtl(pdf,'סיכום פיננסי ל־25 שנה',x+w-7,y+9,4.3,'bold',C.orange);const items=[['הכנסה מצטברת',money(p.totalIncome25)],['השקעה',money(v.costWithVat)],['רווח נקי',money(p.netProfit25)],['ROI',`${Math.round(p.roi25)}%`]];items.forEach((it,i)=>{const cx=x+8+i*41.5;ltr(pdf,it[1],cx+18,y+22,4,'bold',C.white,'center');rtl(pdf,it[0],cx+34,y+30,2.7,'normal',[225,233,239]);});}
function step(pdf,x,y,w,h,n,titleText,text,icon){card(pdf,x,y,w,h,5);pdf.setFillColor(...C.navy);pdf.circle(x+w-10,y+h/2,6,'F');ltr(pdf,String(n),x+w-10,y+h/2+1.8,5,'bold',C.white,'center');iconDraw(pdf,icon,x+11,y+h/2);rtl(pdf,titleText,x+w-22,y+10,5,'bold',C.navy);rtl(pdf,text,x+w-22,y+18,3.4,'normal',C.grey,w-45);}
function whatsappBand(pdf,x,y,w,h){round(pdf,x,y,w,h,C.navy,C.navy,5);iconDraw(pdf,'whatsapp',x+11,y+h/2,C.green);rtl(pdf,'דוח מקצועי ומוכן לשיתוף',x+w-8,y+8,4.8,'bold',C.white);rtl(pdf,'הקובץ כולל את כל הנתונים, ההנחות והתחזית הפיננסית.',x+w-8,y+14.5,3.4,'normal',[230,237,242],w-27);}
function contactBand(pdf,x,y,w,h){round(pdf,x,y,w,h,C.navy,C.navy,5);rtl(pdf,'Solatrix Energy',x+w-8,y+8,4.3,'bold',C.orange);ltr(pdf,'solatrix.energy',x+8,y+13,3.6,'bold',C.orange,'left');}
...
function rtl(pdf,text,x,y,size,style='normal',color=C.navy,maxWidth){pdf.setFont('Heebo',style);pdf.setFontSize(mm(size));pdf.setTextColor(...color);pdf.setR2L(true);const lines=maxWidth?pdf.splitTextToSize(String(text??''),maxWidth):String(text??'');pdf.text(lines,x,y,{align:'right'});pdf.setR2L(false);}
